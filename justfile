...
bazel-lockfile-check:
  ...
  git diff --binary -- Cargo.lock Cargo.Bazel.lock Cargo.Prost.lock Cargo.Prost.Bazel.lock MODULE.bazel.lock > "$before"; \
  CARGO_BAZEL_REPIN=1 bazel {{bazel_startup_flags}} mod tidy; \
  git diff --binary -- Cargo.lock Cargo.Bazel.lock Cargo.Prost.lock Cargo.Prost.Bazel.lock MODULE.bazel.lock > "$after"; \
  if ! cmp -s "$before" "$after"; then \
    printf 'Bazel lockfiles are stale. Keep the changes generated by `CARGO_BAZEL_REPIN=1 bazel mod tidy`.\n'; \
    git diff -- Cargo.lock Cargo.Bazel.lock Cargo.Prost.lock Cargo.Prost.Bazel.lock MODULE.bazel.lock; \
    exit 1; \
  fi
