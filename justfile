...
bazel-lockfile-check:
  ...
  git diff --binary -- Cargo.lock MODULE.bazel.lock > "$before"; \
  bazel {{bazel_startup_flags}} mod tidy --lockfile_mode=update; \
  git diff --binary -- Cargo.lock MODULE.bazel.lock > "$after"; \
  if ! cmp -s "$before" "$after"; then \
    printf 'Bazel lockfiles are stale. Keep the changes generated by `bazel mod tidy`.\n'; \
    git diff -- Cargo.lock MODULE.bazel.lock; \
    exit 1; \
  fi
